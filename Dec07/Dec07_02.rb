...
card_values = %w[J 2 3 4 5 6 7 8 9 T Q K A]
...
hands = source.map do |line|
  ...
  if jokers = grouped.delete('J')
    largest_match_count = grouped.values.max
    biggest_match = grouped.map { |(k, v)| k if v == largest_match_count }.compact.pop
    grouped[biggest_match] = grouped[biggest_match] + jokers
  end
