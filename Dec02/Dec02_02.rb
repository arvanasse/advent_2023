...
games = source.map do |line|
  ...
  OpenStruct.new(game: md[1].to_i, rounds: rounds, possible: false, red: 0, green: 0, blue: 0, power: 0)
end
...
games.each do |game|
  ...
  game.rounds.each do |round|
    round.pulls.each { |pull| game.send("#{pull.color}=", [game.send(pull.color), pull.quantity].max) }
  end
...
puts "Sum of ALL games #{games.reduce(0) { |sum, game| sum + game.power } }" 
puts "Sum of possible games #{possible_games.reduce(0) { |sum, game| sum + game.power } }" 
